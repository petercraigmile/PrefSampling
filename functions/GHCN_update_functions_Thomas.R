...
z_xy <- function(x,y, parents, omega2, delta){
  ...
  delta*sum(dnorm(sqrt((rep(x,nrow(parents))-parents[,1])^2 + (rep(y,nrow(parents))-parents[,2])^2), sd = sqrt(omega2)))
}
...
update_alpha <- function(int, Z, obs_cell, tau2, W, mu_W,
                         prior_mean = 0, prior_var = 50){
    p.inv <- solve(tau2*crossprod(int[obs_cell], int[obs_cell]) + 1/prior_var)
    q <- tau2*crossprod(int[obs_cell], (Z - W[obs_cell] - mu_W)) + prior_mean/prior_var
    new_alpha <- rnorm(1, mean = p.inv*q, sd = sqrt(p.inv))
    new_alpha
}


update_tau2 <- function(Z, int, alpha, W, mu_W, # parameters that are being updated
                        obs_cell, # fixed values 
                        prior_shape = 0.01, prior_rate = 0.01){ # priors
  Y_obs <- alpha*int[obs_cell] + W[obs_cell] + mu_W
...
update_mu_w <- function(Z, int, alpha, W, tau2, obs_cell,
                        prior_mean = 0, prior_var = 25){
  temp <- Z - W[obs_cell] - alpha*int[obs_cell]
...
f_log_phi_w <- function(W, log_phi_W, sig2_W,
  ...
  t2 <- dgamma(exp(log_phi_W), 0.33*5, 5, log = T)
  t1 + t2
}
...
library(mvtnorm)
update_W <- function(curr_W, alpha, Z, int,
                       mu_W, tau2,
  ...
  for(k in 1:nrow(D_grid)){
    id <- which(obs_cell == k)
    if(length(id) != 0){
      sum_obs[k] <- sum(Z[id] - alpha*int[k] - mu_W)
    }
  }
...
f_delta <- function(delta, omega2, W, mu_W, alpha, tau2, Z, S, parents,
  ...
  int <- (lambda(S, parents, delta, omega2))
  # From the observations:
  t1 <- sum(dnorm(Z, alpha*int[obs_cell] + mu_W + W[obs_cell], sqrt(tau2), log = T))
  # From the locations (points):
  t2 <- sum(dpois(N_pts, cell_size*int, log = T))
  # Prior
